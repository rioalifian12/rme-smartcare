<section class="w-full lg:w-5xl bg-gray-50 dark:bg-gray-900 px-8 py-4">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Data User</h1>
    </div>
                    ...
                    <tbody>
                        ...
                        <tr class="border-b border-gray-300 dark:border-gray-700">
                            ...
                            <td class="px-4 py-3">
                                ...
                                @else
                                    <span class="bg-red-100 text-red-800 text-xs font-semibold px-2.5 py-0.5 rounded dark:bg-red-900 dark:text-red-300">Non-Aktif</span>
                                @endif
                            </td>
                            <td class="px-4 py-3 flex items-center space-x-2">
                                {{-- Tombol Edit --}}
                                <button wire:click="openEditModal({{ $user->id }})" title="Edit User" class="flex items-center justify-center text-warning hover:text-warning-strong shadow-xs font-medium leading-5 rounded-base text-sm p-1.5 focus:outline-none cursor-pointer">
                                ...
                                <button wire:click="openResetModal({{ $user->id }})" title="Reset Password" class="flex items-center justify-center text-blue-600 hover:text-blue-800 shadow-xs font-medium leading-5 rounded-base text-sm p-1.5 focus:outline-none cursor-pointer">
                                ...
                                {{-- Tombol Hapus --}}
                                <button wire:click="openDeleteModal({{ $user->id }})" title="Hapus User" class="flex items-center justify-center text-danger hover:text-danger-strong shadow-xs font-medium leading-5 rounded-base text-sm p-1.5 focus:outline-none cursor-pointer">
            ...
            <div class="relative bg-white rounded-lg shadow-xl dark:bg-gray-800 p-4 sm:p-5">
                ...
                <form wire:submit.prevent="createUser" class="space-y-4">
                    <div>
                        ...
                        <input type="text" wire:model="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-base block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white" placeholder="Nama user..." required>
                        @error('name') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                    </div>
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Email</label>
                        <input type="email" wire:model="email" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-base block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white" placeholder="user@rme.com" required>
                        @error('email') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                    </div>
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Password</label>
                        <input type="password" wire:model="password" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-base block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white" placeholder="••••••••" required>
                        @error('password') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                    </div>
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Role</label>
                        <select wire:model="role_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-base block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white" required>
                    ...
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Poliklinik (Opsional)</label>
                        <select wire:model="poly_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-base block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                            <option value="">-- Pilih Poliklinik --</option>
                            @foreach($polyclinics as $p)
                                <option value="{{ $p->id }}">{{ $p->poly_name }}</option>
                            @endforeach
                        </select>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-xs">
        <div class="relative w-full max-w-md max-h-full">
            <div class="relative bg-white rounded-lg shadow-xl dark:bg-gray-800 p-4 sm:p-5">
                <div class="flex justify-between items-center pb-4 mb-4 border-b dark:border-gray-600">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Edit Data User</h3>
                    <button type="button" wire:click="resetInputs" class="text-gray-400 hover:bg-gray-200 text-sm px-1 rounded-full dark:hover:bg-gray-600 cursor-pointer">✕</button>
                </div>
                <form wire:submit.prevent="updateUser" class="space-y-4">
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama Lengkap</label>
                        <input type="text" wire:model="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-base block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white" required>
                        @error('name') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                    </div>
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Email</label>
                        <input type="email" wire:model="email" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-base block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white" required>
                        @error('email') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                    </div>
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Role</label>
                    ...
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Status Akun</label>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-xs">
        <div class="relative w-full max-w-md max-h-full">
            <div class="relative bg-white rounded-lg shadow-xl dark:bg-gray-800 p-4 sm:p-5">
                <div class="flex justify-between items-center pb-4 mb-4 border-b dark:border-gray-600">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Reset Password User</h3>
                    <button type="button" wire:click="resetInputs" class="text-gray-400 hover:bg-gray-200 text-sm px-1 rounded-full dark:hover:bg-gray-600 cursor-pointer">✕</button>
                </div>
                <form wire:submit.prevent="resetUserPassword" class="space-y-4">
                    <div>
                        ...
                        <input type="password" wire:model="reset_password" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-base block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white" placeholder="Masukkan password baru..." required>
                        @error('reset_password') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                    </div>
                    <div class="flex justify-end space-x-3 pt-2">
                        <button type="button" wire:click="resetInputs" class="text-body bg-neutral-secondary-medium box-border border border-default-medium hover:bg-neutral-tertiary-medium hover:text-heading focus:ring-4 focus:ring-neutral-tertiary shadow-xs font-medium leading-5 rounded-base text-sm px-4 py-2.5 focus:outline-none cursor-pointer">Batal</button>
                        <button type="submit" class="text-white bg-brand box-border border border-transparent hover:bg-brand-strong focus:ring-4 focus:ring-brand-medium shadow-xs font-medium leading-5 rounded-base text-sm px-4 py-2.5 focus:outline-none cursor-pointer">Reset Password</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-xs">
        <div class="relative p-4 w-full max-w-md h-full md:h-auto">
            <div class="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                ...
                <p class="mb-4 text-gray-900 dark:text-gray-900">Apakah Anda yakin ingin menghapus user ini?</p>
                <div class="flex justify-center items-center space-x-4">
                    <button type="button" wire:click="resetInputs" class="text-body bg-neutral-secondary-medium box-border border border-default-medium hover:bg-neutral-tertiary-medium hover:text-heading focus:ring-4 focus:ring-neutral-tertiary shadow-xs font-medium leading-5 rounded-base text-sm px-4 py-2.5 focus:outline-none cursor-pointer">Batal</button>
                    <button type="button" wire:click="deleteUser" class="text-white bg-danger box-border border border-transparent hover:bg-danger-strong focus:ring-4 focus:ring-danger-medium shadow-xs font-medium leading-5 rounded-base text-sm px-4 py-2.5 focus:outline-none cursor-pointer">Ya, Hapus</button>
                </div>
            </div>
        </div>
    </div>
